...
static CHAR g_ux_pool_memory[65535];
/*******************************************************************************************************************//**
...
#if !defined(cb_host_event)
    /* Call user function back for USBX Host Class event notification. */
    status = cb_host_event (event, host_class, instance);
    if (UX_SUCCESS != status)
    {
        return status;
    }
...
void ux_common_init0(void)
{
    ...
    status_ux_init = ux_system_initialize ((CHAR *) g_ux_pool_memory, 65535, UX_NULL, 0);
